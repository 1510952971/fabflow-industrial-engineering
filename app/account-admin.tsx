...
export function AccountAdminPanel({ projectId, notify }: { projectId: string; notify: (message: string) => void }) {
  ...
  return <section className="card accountAdmin">
    ...
    <div className="accountRows">{users.map(({ user, assignments }) => {
      ...
      const deleted = user.userStatus === "deleted";
      return <article key={user.userId} className={deleted ? "deleted" : user.userStatus === "active" ? "" : "disabled"}>
        <div className="accountIdentity"><i>{user.displayName.slice(0, 2).toUpperCase()}</i><span><b>{user.displayName}</b><small>{user.email} · {user.hasPassword ? "密码账号" : "工作区账号"}</small></span><em>{user.userStatus === "active" ? "有效" : deleted ? "已删除" : "已停用"}</em></div>
        <div className="accountRoles">{assignments.length ? assignments.map((assignment) => <span key={assignment.roleId ?? `${assignment.role}-${assignment.scopeId}`}><b>{roleLabels[assignment.role ?? ""] ?? assignment.role}</b><small>{assignment.scopeType === "global" ? "全部项目" : assignment.scopeId}</small><button disabled={busy || deleted || (user.userId === currentUserId && assignment.role === "platform_admin")} title="移除此角色" onClick={() => void post({ action: "revoke", userId: user.userId, role: assignment.role, scopeType: assignment.scopeType, scopeId: assignment.scopeId }, "角色已移除")}>×</button></span>) : <em>未授权</em>}</div>
        <div className="accountGrant"><select disabled={deleted} value={grantValue.role} onChange={(event) => setGrant({ ...grant, [user.userId]: { ...grantValue, role: event.target.value } })}>{roles.map((role) => <option key={role} value={role}>{roleLabels[role] ?? role}</option>)}</select><select value={grantValue.scopeType} disabled={deleted || grantValue.role === "platform_admin"} onChange={(event) => setGrant({ ...grant, [user.userId]: { ...grantValue, scopeType: event.target.value } })}><option value="project">当前项目</option><option value="global">全部项目</option></select><button disabled={busy || deleted} onClick={() => void post({ action: "grant", userId: user.userId, role: grantValue.role, scopeType: grantValue.scopeType, scopeId: grantValue.scopeType === "project" ? projectId : "*" }, "角色已授予")}>授予角色</button></div>
        <div className="accountActions"><button disabled={busy || deleted} onClick={() => { setResetUser({ id: user.userId, name: user.displayName }); setResetPassword(""); }}>重置密码</button><button disabled={busy || deleted || user.userId === currentUserId} className={user.userStatus === "active" ? "danger" : ""} onClick={() => void post({ action: "set_status", userId: user.userId, status: user.userStatus === "active" ? "disabled" : "active" }, user.userStatus === "active" ? "账号已停用并注销全部会话" : "账号已启用")}>{user.userStatus === "active" ? "停用账号" : "启用账号"}</button><button disabled={busy || deleted || user.userId === currentUserId} className="danger" onClick={() => { if (window.confirm(`确认删除 ${user.displayName}？\n\n账号将无法登录，角色和密码会被移除，历史审计记录保留。`)) void post({ action: "delete_user", userId: user.userId }, "账号已删除，历史审计记录已保留"); }}>删除账号</button></div>
      </article>;
    })}</div>
