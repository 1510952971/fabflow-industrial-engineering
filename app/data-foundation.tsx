...
function SourceView({ sources, syncing, syncSource, notify }: { sources: Source[]; syncing: string | null; syncSource: (id: string) => void; notify: Notify }) {
  return <div className="dataGrid"><section className="card sourceBoard"><div className="dataHead"><div><span>SOURCE CONNECTORS</span><h3>企业数据源与同步健康度</h3><p>每个连接器都有负责人、同步频率、游标和失败重试记录。</p></div><button onClick={() => notify("失败队列查看需要先配置服务端集成凭据")}>查看失败队列</button></div><div className="sourceRows">{sources.map((s) => <div className="sourceRow" key={s.id}><i className={`sourceIcon ${s.status === "健康" ? "good" : s.status === "关注" ? "warn" : "idle"}`}>{s.id.slice(0, 2)}</i><span><b>{s.name}</b><small>{s.owner} · {s.mode}</small></span><div className="sourceCount"><b>{s.records}</b><small>记录</small></div><div><em className={`sourceStatus ${s.status === "健康" ? "good" : s.status === "关注" ? "warn" : "idle"}`}>{s.status}</em><small>{s.lastSync} · {s.cadence}</small></div><button className="syncButton" disabled={syncing === s.id} onClick={() => syncSource(s.id)}>{syncing === s.id ? "检查中…" : s.status === "待配置" ? "检查配置" : "检查连接"}</button></div>)}</div></section><aside className="card eventQueue"><span>EVENT QUEUE</span><h3>事件总线</h3><div className="queueRing"><b>87%</b><small>处理成功率</small></div><p><span>待处理</span><b>42</b></p><p><span>重试中</span><b className="amberText">7</b></p><p><span>死信队列</span><b className="redText">2</b></p><button className="fullPrimary" onClick={() => notify("事件队列由服务端集成密钥保护，当前页面不伪造队列数据")}>查看事件详情</button></aside><section className="card dataContract"><div className="dataHead"><div><span>CANONICAL MODEL</span><h3>统一对象与数据契约</h3></div><button onClick={copySchema}>复制 JSON Schema</button></div><div className="contractGrid">{[["Project","项目主键、阶段、厂区"],["System","系统边界、负责人、状态"],["Tag / Interface","Tag、接口、Cause & Effect"],["Material / PO","物料、供应商、订单与收货"],["Test Pack / Punch","测试证据、NCR、Punch"],["Attachment / Audit","文件版本、签名、审计事件"]].map(([name,desc]) => <div key={name}><code>{name}</code><p>{desc}</p><i>→</i></div>)}</div></section></div>;
}
...
function EvidenceView({ notify }: { notify: Notify }) {
  const exportAudit=()=>{downloadCsv(`fabflow-audit-${Date.now()}.csv`,["审计编号","操作者","动作","对象","时间","状态"],auditRows);notify("审计清单已下载")};
  return <div className="dataGrid"><section className="card auditBoard"><div className="dataHead"><div><span>R2 EVIDENCE & AUDIT</span><h3>附件证据链与不可抵赖审计</h3><p>文件以对象为中心挂接，版本、签署人、来源、哈希和审批事件形成完整证据链。</p></div><div><button onClick={exportAudit}>导出审计</button><button onClick={() => openMasterData("projects")}>选择对象并上传附件</button></div></div><div className="auditRows">{auditRows.map((r) => <div className="auditRow" key={r[0]}><code>{r[0]}</code><span><b>{r[2]}</b><small>{r[1]} · {r[3]}</small></span><time>{r[4]}</time><em className={r[5] === "待审" ? "warn" : "good"}>{r[5]}</em></div>)}</div></section><aside className="card retentionBoard"><span>RETENTION POLICY</span><h3>保留与归档策略</h3><div className="retentionRing"><b>7 年</b><small>关键工程记录</small></div><p><span>设计输入 / 计算书</span><b>永久 + 版本</b></p><p><span>测试 / 签字记录</span><b>7 年</b></p><p><span>采购 / 收货记录</span><b>10 年</b></p><p><span>运行趋势数据</span><b>2 年热存</b></p><button className="fullPrimary" onClick={() => notify("归档策略由项目管理员维护，当前只读展示")}>查看归档策略</button></aside></div>;
}
