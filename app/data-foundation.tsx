...
function GovernanceView({ notify }: { notify: Notify }) {
  return <div className="dataGrid"><section className="card roleBoard"><div className="dataHead"><div><span>RBAC MATRIX</span><h3>按项目、系统、采购包分层授权</h3><p>权限不能只靠前端按钮隐藏，最终必须在服务端按对象和动作校验。</p></div><button onClick={() => void copyText(roleRows.map(row=>row.join("\t")).join("\n")).then(()=>notify("角色模板已复制")).catch(error=>notify(error instanceof Error?error.message:"复制失败"))}>复制角色模板</button></div><div className="roleTable"><table><thead><tr><th>角色</th><th>可操作范围</th><th>审批动作</th><th>数据范围</th></tr></thead><tbody>{roleRows.map((r) => <tr key={r[0]}><td><b>{r[0]}</b></td><td>{r[1]}</td><td><i className="permissionTag">{r[2]}</i></td><td>{r[3]}</td></tr>)}</tbody></table></div></section><aside className="card approvalBoard"><span>WORKFLOW QUEUE</span><h3>审批链与 SLA</h3>{workflowRows.map((r) => <button className="approvalRow" key={r[0]} onClick={() => {openMasterData("workflowActions",r[0]);notify(`${r[0]} 审批详情已打开`)}}><span><b>{r[0]}</b><small>{r[1]}</small></span><em>{r[2]}</em><i>{r[3]}</i></button>)}<button className="fullPrimary" onClick={() => {openMasterData("workflowActions","approval_workflow");notify("审批流配置数据已打开")}}>配置审批流</button></aside></div>;
}
