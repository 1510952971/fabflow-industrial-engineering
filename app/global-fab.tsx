...
export function GlobalFabPage({notify}:{notify:Notify}){
  ...
  return <>
    <PageTop actions={<><button className="softButton" onClick={()=>notify("已生成全球项目周报")}>生成周报</button><button className="primaryButton" onClick={()=>notify("新 FAB 项目向导已创建")}>＋ 新建 FAB 项目</button></>}/>
  ...
  return <>
    ...
    <div className="portfolioLayout">
      ...
      <aside className="card projectFocus">
        ...
        <button className="fullPrimary" onClick={()=>notify(`已打开 ${current.name} 项目控制室`)}>进入项目控制室 <span>→</span></button>
      </aside>
    </div>
    <section className="card gateBoard"><div className="globalSectionHead"><div><span>STAGE-GATE GOVERNANCE</span><h3>全球统一阶段闸门</h3></div><button onClick={()=>notify("阶段闸门模板已打开")}>管理闸门模板 →</button></div><div className="gateFlow">{gates.map((g,i)=><div className={i<3?"complete":i===3?"current":""} key={g.id}><div className="gateNode"><i>{i<3?"✓":g.id}</i><span><b>{g.name}</b><small>{g.short}</small></span></div><p>{g.deliverables} 项交付物</p><em>{g.owner}</em></div>)}</div></section>
  </>;
}
...
function StandardsView({region,setRegion,notify}:{region:string;setRegion:(x:string)=>void;notify:Notify}){
  ...
  return <>
    ...
    <div className="standardsGrid">{list.map((p,i)=><section className="card standardPack" key={p.code}><div className={`standardBadge ${p.color}`}>{p.code}</div><span>STANDARD PACKAGE 0{i+1}</span><h3>{p.name}</h3><div>{p.items.map(x=><p key={x}><i>✓</i>{x}</p>)}</div><footer><span><i className="goodDot"/>版本已冻结</span><button onClick={()=>notify(`${p.name} 已打开`)}>查看标准包 →</button></footer></section>)}</div>
    <div className="standardBottom"><section className="card gateChecklist"><div className="globalSectionHead"><div><span>GATE READINESS</span><h3>G3 详细设计启动条件</h3></div><div className="readiness"><b>67%</b><span><i style={{width:"67%"}}/></span></div></div>{checks.map((x,i)=><div className="checkRow" key={x[0]}><i className={i<2?"done":i<4?"progress":""}>{i<2?"✓":i<4?"•":""}</i><b>{x[0]}</b><span>{x[1]}</span><em className={i<2?"done":i<4?"progress":""}>{x[2]}</em><button onClick={()=>notify(`已打开：${x[0]}`)}>查看证据</button></div>)}</section><aside className="card deviationCard"><span>STANDARD DEVIATION</span><h3>标准偏差申请</h3><div className="deviationRing"><b>7</b><small>开放申请</small></div><p><span>高风险</span><b>1</b></p><p><span>评审中</span><b>4</b></p><p><span>待项目总工批准</span><b>2</b></p><button className="fullPrimary" onClick={()=>notify("新标准偏差申请已创建")}>＋ 新建偏差申请</button></aside></div>
  </>;
}
