...
export function GlobalFabPage({notify:parentNotify}:{notify:Notify}){
  ...
  return <>
    <PageTop actions={<><button className="softButton" onClick={()=>{downloadCsv("fabflow-global-fab-weekly-report.csv",["项目","区域","阶段","进度","开放问题"],fabs.map(f=>[f.name,f.region,f.stage,`${f.progress}%`,f.open]));notify("已生成全球项目周报")}}>生成周报</button><button className="primaryButton" onClick={()=>{openMasterData("projects");notify("已打开新 FAB 项目表单")}}>＋ 新建 FAB 项目</button></>}/>
...
function CollaborationView({rfis:rawRfis,setRfis,notify}:{rfis:typeof initialRfis;setRfis:(x:typeof initialRfis)=>void;notify:Notify}){
  const {states,save}=useWorkflowStates("rfi_state");
  const rfis=rawRfis.map((item)=>({...item,status:states[item.id]?.status??item.status}));
  const [filter,setFilter]=useState("全部");
  const shown=rfis.filter(r=>filter==="全部"||r.status===filter);
  const resolve=async(id:string)=>{const item=rfis.find((entry)=>entry.id===id);if(!item)return;try{await save(id,"已答复",item,item.title,"rfi");setRfis(rawRfis.map(r=>r.id===id?{...r,status:"已答复"}:r));notify(id+" 已关闭并同步至责任专业")}catch(error){notify(error instanceof Error?error.message:"RFI 状态保存失败")}};
  return <div className="collabLayout">
    <section className="card coordinationMatrix"><div className="globalSectionHead"><div><span>DESIGN COORDINATION</span><h3>跨专业设计成熟度矩阵</h3></div><button onClick={()=>{openMasterData("workflowActions","模型联审");notify("已打开本周模型联审任务表单")}}>发起联审</button></div><div className="matrixTable"><table><thead><tr><th>专业系统</th><th>30% 模型</th><th>60% 模型</th><th>90% 模型</th><th>IFC 发布</th><th>开放冲突</th></tr></thead><tbody>{[
...
function StandardsView({region,setRegion,notify}:{region:string;setRegion:(x:string)=>void;notify:Notify}){
  ...
  return <>
    ...
    <div className="standardBottom"><section className="card gateChecklist"><div className="globalSectionHead"><div><span>GATE READINESS</span><h3>G3 详细设计启动条件</h3></div><div className="readiness"><b>67%</b><span><i style={{width:"67%"}}/></span></div></div>{checks.map((x,i)=><div className="checkRow" key={x[0]}><i className={i<2?"done":i<4?"progress":""}>{i<2?"✓":i<4?"•":""}</i><b>{x[0]}</b><span>{x[1]}</span><em className={i<2?"done":i<4?"progress":""}>{x[2]}</em><button onClick={()=>{openMasterData("workflowActions",String(x[0]));notify(`已打开：${x[0]}`)}}>查看证据</button></div>)}</section><aside className="card deviationCard"><span>STANDARD DEVIATION</span><h3>标准偏差申请</h3><div className="deviationRing"><b>7</b><small>开放申请</small></div><p><span>高风险</span><b>1</b></p><p><span>评审中</span><b>4</b></p><p><span>待项目总工批准</span><b>2</b></p><button className="fullPrimary" onClick={()=>{openMasterData("workflowActions","standard_deviation");notify("已打开新标准偏差申请表单")}}>＋ 新建偏差申请</button></aside></div>
  </>;
}

function DocumentsView({approved:localApproved,setApproved,notify}:{approved:string[];setApproved:(x:string[])=>void;notify:Notify}){
  const {states,save}=useWorkflowStates("document_signoff");
  const approved=[...new Set([...localApproved,...Object.values(states).filter((state)=>state.status==="已批准").map((state)=>state.entityId)])];
  const approve=async(no:string)=>{const document=documents.find((item)=>item.no===no);if(!document)return;try{await save(no,"已批准",document,document.title,"controlled_document");setApproved([...localApproved,no]);notify(no+" 已完成电子会签")}catch(error){notify(error instanceof Error?error.message:"会签状态保存失败")}};
  return <>
