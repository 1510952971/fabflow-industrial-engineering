...
function CalculationPage({notify}:{notify:Notify}) {
  ...
  const exportCalculation=()=>{downloadText(`fastener-calculation-${Date.now()}.txt`,[`Facility Construction Management Software 紧固件计算书`,`结构：${layers}`,`材质：${material}`,`板厚：${thickness} mm`,`推荐规格：${bolt}`,`建议扭矩：${torque} N·m`,`计算数量：${total} 套`,`采购数量（含 5%）：${Math.ceil(total*1.05)} 套`,"",...rows.map((row,index)=>`${index+1}. ${row.area} / ${row.structure} / ${row.thickness}mm / ${row.points} 点`)].join("\r\n"));notify("计算书已下载")};
...
function BomPage({notify}:{notify:Notify}) {
  ...
  return <>
    <PageTop eyebrow="BILL OF MATERIALS" title="BOM 成本报表" desc="统一管理物料数量、采购余量与成本分项，支持多格式专业交付" actions={<><button className="exportButton" onClick={()=>void submitProjectRelease()}>提交项目发布审批</button><button className="exportButton" onClick={()=>void copyCad()}>复制 CAD 文本</button><button className="exportButton" onClick={()=>downloadText(`fabflow-bom-note-${Date.now()}.txt`,`Facility Construction Management Software 项目 BOM 设计说明\n物料条目：${shown.length}\n采购余量：${margin}%\n成本合计：CNY ${sum.toFixed(2)}\n导出时间：${new Date().toISOString()}`)}>生成设计说明</button><button className="primaryButton" onClick={exportBom}>导出 Excel / CSV</button></>}/>
